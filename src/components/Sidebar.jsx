...
export default function Sidebar({ open, setOpen }) {
  ...
  return (
    <>
      ...
      <div className={`fixed top-0 right-0 z-40 w-64 h-full bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 shadow-2xl transform transition-all duration-500 ease-in-out lg:translate-x-0 lg:static lg:inset-0 sidebar ${open ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex items-center justify-between h-20 px-6 border-b border-slate-700/50">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-white">إدارة المخزون</h1>
              <p className="text-xs text-slate-400">نظام متقدم</p>
            </div>
          </div>
          <button
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-slate-700/50">
          <div className="flex items-center space-x-3 p-3 rounded-xl bg-slate-800/50">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-white">المدير</p>
